...
> [!definition] 定义 1.5.1
> 设 $\mathscr{X}$ 是线性空间，$E\subset\mathscr{X}$，称 $E$ 为**凸集**，如果对于任意 $x,y\in E$ 及 $\lambda\in[0,1]$，有 $\lambda x+(1-\lambda)y\in E$.

> [!example] 例 1.5.2
> 设 $\mathscr{X}$ 是赋范空间，则开球 $B(x_0,r)$ 和闭球 $\overline{B}(x_0,r)$ 都是凸集.

> [!proof]- 证
> 设 $x,y\in B(x_0,r)$，则 $\|x-x_0\|<r$，$\|y-x_0\|<r$。对 $\lambda\in[0,1]$，
> $$\|\lambda x+(1-\lambda)y - x_0\| = \|\lambda(x-x_0)+(1-\lambda)(y-x_0)\|$$
> $$\leqslant \lambda\|x-x_0\|+(1-\lambda)\|y-x_0\| < \lambda r+(1-\lambda)r = r.$$
> 故 $\lambda x+(1-\lambda)y\in B(x_0,r)$。 $\blacksquare$

> [!definition] 定义 1.5.3
> 设 $\mathscr{X}$ 是线性空间，函数 $f:\mathscr{X}\rightarrow\mathbb{R}\cup\{+\infty\}$ 称为**凸函数**，如果
> $$f(\lambda x+(1-\lambda)y) \leqslant \lambda f(x)+(1-\lambda)f(y)$$
> 对所有 $x,y\in\mathscr{X}$ 及 $\lambda\in[0,1]$ 成立.

> [!theorem] 定理 1.5.4 (Jensen 不等式)
> 设 $f$ 是 $\mathbb{R}^n$ 上的凸函数，则对任意 $x_1,x_2,\cdots,x_k\in\mathbb{R}^n$ 及 $\lambda_1,\lambda_2,\cdots,\lambda_k\geqslant 0$ 满足 $\sum_{i=1}^{k}\lambda_i=1$，有
> $$f\left(\sum_{i=1}^{k}\lambda_i x_i\right) \leqslant \sum_{i=1}^{k}\lambda_i f(x_i).$$

> [!proof]- 证
> 用数学归纳法。 $\blacksquare$

> [!theorem] 定理 1.5.5
> 设 $\mathscr{X}$ 是赋范空间，$C$ 是 $\mathscr{X}$ 中的闭凸集。则对 $\forall x\in\mathscr{X}$，存在唯一的 $x_0\in C$，使得
> $$\|x-x_0\| = \inf_{y\in C}\|x-y\|.$$

> [!definition] 定义 1.5.6
> 上述定理中的 $x_0$ 称为 $x$ 在 $C$ 上的**投影**，记作 $P_C x$.

> [!theorem] 定理 1.5.7
> 设 $C$ 是 Hilbert 空间 $H$ 中的闭凸子集，则投影算子 $P_C$ 满足：
> (1) $\|P_C x - P_C y\| \leqslant \|x-y\| \quad (\forall x,y\in H)$；
> (2) $(x-P_C x, y-P_C x) \leqslant 0 \quad (\forall y\in C)$。

### 5.2 Brouwer 不动点定理与 Schauder 不动点定理

> [!theorem] 定理 1.5.8 (Brouwer 不动点定理)
> 设 $B^n$ 是 $\mathbb{R}^n$ 中的闭单位球，$T:B^n\rightarrow B^n$ 是连续映射，则 $T$ 至少有一个不动点，即存在 $x\in B^n$，使得 $Tx=x$.

我们不证明这个定理，有兴趣的读者可以参考有关的拓扑学书籍。

Brouwer 不动点定理的一个重要推广是 Schauder 不动点定理.

> [!theorem] 定理 1.5.9 (Schauder 不动点定理)
> 设 $C$ 是 Banach 空间 $\mathscr{X}$ 中的一个闭凸子集，$T:C\rightarrow C$ 是连续映射，且 $T(C)$ 是列紧集，则 $T$ 在 $C$ 中至少有一个不动点.

> [!proof]- 证
> 设 $K=\overline{T(C)}$，则 $K$ 是紧集。$\forall n\in\mathbb{N}$，存在 $K$ 的有限 $1/n$ 网 $N_n=\{x_1^{(n)},x_2^{(n)},\cdots,x_{k_n}^{(n)}\}$。
>
> 令 $E_n=\operatorname{span}N_n$，则 $E_n$ 是有限维子空间。定义映射 $I_n:K\rightarrow\operatorname{co}(N_n)$ 如下：
> $$I_n(x) = \frac{\sum_{i=1}^{k_n}\mu_i^{(n)}(x)x_i^{(n)}}{\sum_{i=1}^{k_n}\mu_i^{(n)}(x)},$$
> 其中
> $$\mu_i^{(n)}(x) = \max\left\{0,\frac{1}{n}-\|x-x_i^{(n)}\|\right\}.$$
>
> 容易验证 $I_n$ 是连续的，且 $\|I_n(x)-x\|<1/n$ 对所有 $x\in K$ 成立。
>
> 令 $T_n = I_n\circ T$，则 $T_n$ 将 $C\cap E_n$ 映入 $C\cap E_n$（因为 $C$ 是凸的）。由 Brouwer 不动点定理，$T_n$ 有不动点 $x_n\in C\cap E_n$，即 $T_n x_n = x_n$。
>
> 由于 $\{x_n\}\subset C$ 且 $T(C)$ 列紧，$\{Tx_n\}$ 有收敛子列。不妨设 $Tx_n\rightarrow y\in K$。于是
> $$\|x_n-y\| = \|T_n x_n - y\| \leqslant \|T_n x_n - Tx_n\| + \|Tx_n - y\|$$
> $$= \|I_n(Tx_n)-Tx_n\| + \|Tx_n-y\| < \frac{1}{n} + \|Tx_n-y\| \rightarrow 0.$$
> 因此 $x_n\rightarrow y$。由 $T$ 的连续性，$Ty = y$，即 $y$ 是 $T$ 的不动点。 $\blacksquare$

> [!corollary] 推论 1.5.10
> 设 $C$ 是 Banach 空间 $\mathscr{X}$ 中的有界闭凸集，$T:C\rightarrow C$ 是紧算子（即 $T$ 连续且 $T(C)$ 列紧），则 $T$ 在 $C$ 中至少有一个不动点.

> [!example] 例 1.5.11
> 考虑积分方程
> $$u(x) = \lambda\int_a^b K(x,y)u(y)\mathrm{d}y + f(x),$$
> 其中 $K(x,y)$ 在 $[a,b]\times[a,b]$ 上连续，$f\in C[a,b]$。当 $|\lambda|$ 充分小时，这个方程在 $C[a,b]$ 中有唯一解.

## 第二章 线性算子与线性泛函

线性算子是线性空间到线性空间的线性映射。当相空间是数域时，线性算子称为线性泛函。线性算子和线性泛函是泛函分析研究的基本对象。
...
#### 1.1 线性算子的定义
> [!definition] 定义 2.1.1
> 设 $\mathscr{X},\mathscr{Y}$ 是两个线性空间, D 是 $\mathscr{X}$ 的一个线性子空间.$T:D\rightarrow\mathscr{Y}$ 是一种映射,D 称为 T 的**定义域**,有时记作 D(T).值域 $R(T)=\{Tx|x\in D\}$ 称为 T 的**值域**.如果
> $$T(\alpha x+\beta y) = \alpha Tx + \beta Ty \quad (\forall x,y\in D,\forall\alpha,\beta\in\mathbb{K}),$$
> 那么 T 就称为一个**线性算子**.
> [!example] 例 2.1.2
> 设 $\mathscr{X}=\mathbb{R}^n$,$\mathscr{Y}=\mathbb{R}^m$,$x=(x_1,x_2,\cdots,x_n)$.矩阵 $A=(a_{ij})_{m\times n}$ 定义了一个线性算子
> $$y = Ax,$$
> 其中 $y=(y_1,y_2,\cdots,y_m)$,$y_i=\sum_{j=1}^{n}a_{ij}x_j(i=1,2,\cdots,m)$.
> [!example] 例 2.1.3
> (积分算子) 设 $K(x,y)$ 是 $[a,b]\times[a,b]$ 上的连续函数.定义算子 $T:C[a,b]\rightarrow C[a,b]$ 如下:
> $$(Tu)(x) = \int_a^b K(x,y)u(y)\mathrm{d}y \quad (u\in C[a,b]).$$
> 容易验证 T 是线性的.
> (此页内容与第37页相同,为空白或重复页)
> [!example] 例 2.1.4
> (微分算子) 设 $\mathscr{X}=C^1[a,b]$,$\mathscr{Y}=C[a,b]$,定义算子 T 如下:
> $$Tu = \frac{\mathrm{d}u}{\mathrm{d}t} \quad (u\in C^1[a,b]).$$
> 容易验证 T 是线性算子.
> [!definition] 定义 2.1.5
> 设 $T:\mathscr{X}\rightarrow\mathscr{Y}$ 是线性算子.如果 $T^{-1}:R(T)\rightarrow\mathscr{X}$ 存在,即 T 是单射,则称 $T^{-1}$ 为 T 的**逆算子**.
> [!proposition] 命题 2.1.6
> 线性算子 T 的逆算子 $T^{-1}$ 存在,当且仅当 $Tx=\theta$ 蕴涵 $x=\theta$.
> [!proposition] 命题 2.1.7
> 设 $T:\mathscr{X}\rightarrow\mathscr{Y}$ 是线性算子,$\mathscr{X}$ 和 $\mathscr{Y}$ 是赋范空间.则 T 是连续的,当且仅当 T 将 $\mathscr{X}$ 中的有界集映为 $\mathscr{Y}$ 中的有界集.
> [!proof]- 证
> 必要性.设 T 连续.若 $A\subset\mathscr{X}$ 有界,则 $\exists M>0$,使得 $\|x\|\leqslant M(\forall x\in A)$.假设 $T(A)$ 无界,则 $\forall n\in\mathbb{N}$,$\exists x_n\in A$,使得 $\|Tx_n\|>n$.令 $y_n=x_n/\sqrt{n}$,则 $y_n\rightarrow\theta$.由 T 的连续性,$Ty_n\rightarrow\theta$,但 $\|Ty_n\|=\|Tx_n\|/\sqrt{n}>\sqrt{n}\rightarrow\infty$,矛盾.
> 充分性.设 $x_n\rightarrow\theta$.要证 $Tx_n\rightarrow\theta$.设不然,则 $\exists\varepsilon_0>0$ 及子列 $\{x_{n_k}\}$,使得 $\|Tx_{n_k}\|\geqslant\varepsilon_0$.令 $z_{n_k}=x_{n_k}/\|x_{n_k}\|$,则 $\|z_{n_k}\|=1$,即 $\{z_{n_k}\}$ 有界.由假设,$\{Tz_{n_k}\}$ 有界.但
> $$\|Tz_{n_k}\| = \frac{\|Tx_{n_k}\|}{\|x_{n_k}\|} \geqslant \frac{\varepsilon_0}{\|x_{n_k}\|} \rightarrow +\infty,$$
> 矛盾.$\blacksquare$
#### 1.2 线性算子的有界性
> [!definition] 定义 2.1.8
> 设 $\mathscr{X}$ 和 $\mathscr{Y}$ 是赋范空间,$T:\mathscr{X}\rightarrow\mathscr{Y}$ 是线性算子.称 T 是**有界**的,如果 T 将 $\mathscr{X}$ 中的每个有界集映为 $\mathscr{Y}$ 中的有界集.即对 $\forall M>0$,$\exists K>0$,当 $\|x\|\leqslant M$ 时,
> $$\|Tx\|\leqslant K.$$
> 由命题 2.1.7,线性算子 T 连续当且仅当 T 有界.
> [!proposition] 命题 2.1.9
> 设 T 是有界线性算子,则
> $$\sup_{\|x\|=1}\|Tx\| < +\infty,$$
> 且 $\|Tx\|\leqslant\sup_{\|x\|=1}\|Tx\|\cdot\|x\|(\forall x\in\mathscr{X})$.
> [!proof]- 证
> 单位球面 $S_1=\{x\in\mathscr{X}|\|x\|=1\}$ 是有界集,由 T 的有界性,$\sup_{\|x\|=1}\|Tx\|<+\infty$.对 $x\neq\theta$,
> $$\left\|T\frac{x}{\|x\|}\right\| \leqslant \sup_{\|x\|=1}\|Tx\|,$$
> 即 $\|Tx\|\leqslant\sup_{\|x\|=1}\|Tx\|\cdot\|x\|$.$\blacksquare$
> [!definition] 定义 2.1.10
> 设 $T:\mathscr{X}\rightarrow\mathscr{Y}$ 是有界线性算子,令
> $$\|T\| = \sup_{\|x\|=1}\|Tx\|,$$
> 称为 T 的**算子范数**.
> 容易验证算子范数满足:
> $$\|Tx\| \leqslant \|T\|\cdot\|x\| \quad (\forall x\in\mathscr{X}).$$
> 事实上,当 $x=\theta$ 时上式显然成立.当 $x\neq\theta$ 时,$\|x/\|x\|\|=1$,故
> $$\left\|T\frac{x}{\|x\|}\right\| \leqslant \|T\|,$$
> 即 $\|Tx\|\leqslant\|T\|\cdot\|x\|$.
> 算子范数还可以表示为:
> $$\|T\| = \sup_{\|x\|\leqslant 1}\|Tx\| = \sup_{x\neq\theta}\frac{\|Tx\|}{\|x\|} = \inf\{M\geqslant 0|\|Tx\|\leqslant M\|x\|,\forall x\in\mathscr{X}\}.$$
> [!example] 例 2.1.11
> 设 $\mathscr{X}=\mathbb{R}^n$,$\mathscr{Y}=\mathbb{R}^m$,$T$ 由矩阵 $A=(a_{ij})_{m\times n}$ 定义,即 $Tx=Ax$.则
> $$\|T\| = \sup_{\|x\|=1}\|Ax\|.$$
> 当 $\mathbb{R}^n$ 和 $\mathbb{R}^m$ 都取欧氏范数时,$\|T\|$ 就是矩阵 A 的谱范数,即 A 的最大奇异值.
> [!example] 例 2.1.12
> (Fourier 变换) 设 $f\in L^1(\mathbb{R})$,定义 Fourier 变换
> $$\hat{f}(\xi) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{+\infty}f(x)e^{-ix\xi}\mathrm{d}x.$$
> 可以证明 $\hat{f}\in L^{\infty}(\mathbb{R})$,且 $\|\hat{f}\|_\infty\leqslant\frac{1}{\sqrt{2\pi}}\|f\|_1$.因此 Fourier 变换 $\mathscr{F}:L^1(\mathbb{R})\rightarrow L^{\infty}(\mathbb{R})$ 是有界线性算子,且 $\|\mathscr{F}\|\leqslant\frac{1}{\sqrt{2\pi}}$.
> [!proposition] 命题 2.1.13
> 设 $\mathscr{X},\mathscr{Y}$ 是赋范空间,记 $\mathscr{L}(\mathscr{X},\mathscr{Y})$ 为从 $\mathscr{X}$ 到 $\mathscr{Y}$ 的有界线性算子全体.在 $\mathscr{L}(\mathscr{X},\mathscr{Y})$ 上定义加法和数乘:
> $$(T_1+T_2)x = T_1 x + T_2 x, \quad (\alpha T)x = \alpha(Tx),$$
> 则 $\mathscr{L}(\mathscr{X},\mathscr{Y})$ 是线性空间.再按算子范数定义范数,则 $\mathscr{L}(\mathscr{X},\mathscr{Y})$ 成为赋范空间.
> [!proof]- 证
> 只需验证范数公理.设 $\|T\|=\sup_{\|x\|=1}\|Tx\|$,则:
> (1) $\|T\|\geqslant 0$,且 $\|T\|=0\Leftrightarrow Tx=\theta(\forall x\in\mathscr{X})\Leftrightarrow T=0$;
> (2) $\|\alpha T\| = \sup_{\|x\|=1}\|\alpha Tx\| = |\alpha|\sup_{\|x\|=1}\|Tx\| = |\alpha|\cdot\|T\|$;
> (3) $\|T_1+T_2\| = \sup_{\|x\|=1}\|(T_1+T_2)x\| \leqslant \sup_{\|x\|=1}(\|T_1 x\|+\|T_2 x\|) \leqslant \|T_1\|+\|T_2\|$.$\blacksquare$
> [!theorem] 定理 2.1.14
> 当 $\mathscr{Y}$ 是 B 空间时,$\mathscr{L}(\mathscr{X},\mathscr{Y})$ 也是 B 空间.
> [!proof]- 证
> 设 $\{T_n\}$ 是 $\mathscr{L}(\mathscr{X},\mathscr{Y})$ 中的基本列,即 $\|T_n-T_m\|\rightarrow 0(n,m\rightarrow\infty)$.对 $\forall x\in\mathscr{X}$,
> $$\|T_n x-T_m x\| \leqslant \|T_n-T_m\|\cdot\|x\| \rightarrow 0,$$
> 所以 $\{T_n x\}$ 是 $\mathscr{Y}$ 中的基本列.由于 $\mathscr{Y}$ 完备,存在 $Tx\in\mathscr{Y}$,使得 $T_n x\rightarrow Tx$.
> 这样定义了算子 $T:\mathscr{X}\rightarrow\mathscr{Y}$.容易验证 T 是线性的.又
> $$\|Tx\| = \lim_{n\rightarrow\infty}\|T_n x\| \leqslant \lim_{n\rightarrow\infty}\|T_n\|\cdot\|x\|,$$
> 故 T 有界,即 $T\in\mathscr{L}(\mathscr{X},\mathscr{Y})$.最后,
> $$\|T_n-T\| = \sup_{\|x\|=1}\|T_n x-T x\| = \sup_{\|x\|=1}\lim_{m\rightarrow\infty}\|T_n x-T_m x\| \leqslant \varlimsup_{m\rightarrow\infty}\|T_n-T_m\| \rightarrow 0,$$
> 因此 $T_n\rightarrow T$.$\blacksquare$
> [!definition] 定义 2.1.15
> 特别地,当 $\mathscr{Y}=\mathbb{K}$(数域)时,记 $\mathscr{X}^*=\mathscr{L}(\mathscr{X},\mathbb{K})$,称为 $\mathscr{X}$ 的**共轭空间**(或对偶空间).$\mathscr{X}^*$ 中的元素称为 $\mathscr{X}$ 上的**有界线性泛函**.
> [!example] 例 2.1.16
> 设 $\Omega\subset\mathbb{R}^n$ 是有界开区域,$p,q$ 是共轭指数,即 $1/p+1/q=1$,$1\leqslant p<\infty$.则 $(L^p(\Omega))^* = L^q(\Omega)$.
> 更精确地说,对 $\forall f\in (L^p(\Omega))^*$,存在唯一的 $u\in L^q(\Omega)$,使得
> $$f(v) = \int_\Omega u(x)v(x)\mathrm{d}x \quad (\forall v\in L^p(\Omega)),$$
> 且 $\|f\| = \|u\|_q$.
> 这就是著名的 Riesz 表示定理.
> [!theorem] 定理 2.1.17
> (Hahn-Banach 定理,实形式) 设 $\mathscr{X}$ 是实赋范空间,$E\subset\mathscr{X}$ 是线性子空间,$f_0$ 是 E 上的实线性泛函,满足
> $$f_0(x) \leqslant p(x) \quad (\forall x\in E),$$
> 其中 p 是 $\mathscr{X}$ 上的一个次线性泛函,即满足:
> (1) $p(x+y)\leqslant p(x)+p(y) \quad (\forall x,y\in\mathscr{X})$;
> (2) $p(\alpha x)=\alpha p(x) \quad (\forall\alpha\geqslant 0,\forall x\in\mathscr{X})$.
> 则存在 $\mathscr{X}$ 上的实线性泛函 f,使得 $f(x)=f_0(x)(\forall x\in E)$,且
> $$f(x) \leqslant p(x) \quad (\forall x\in\mathscr{X}).$$
> [!theorem] 定理 2.1.18
> (Hahn-Banach 定理,赋范空间形式) 设 $\mathscr{X}$ 是赋范空间,$E\subset\mathscr{X}$ 是线性子空间,$f_0$ 是 E 上的有界线性泛函.则存在 $\mathscr{X}$ 上的有界线性泛函 f,使得
> $$f(x) = f_0(x) \quad (\forall x\in E),$$
> 且 $\|f\| = \|f_0\|_E$,其中 $\|f_0\|_E = \sup_{x\in E,\|x\|=1}|f_0(x)|$.
> [!proof]- 证
> 令 $p(x)=\|f_0\|_E\cdot\|x\|$,则 p 是次线性泛函,且
> $$f_0(x) \leqslant |f_0(x)| \leqslant \|f_0\|_E\cdot\|x\| = p(x) \quad (\forall x\in E).$$
> 由 Hahn-Banach 定理(实形式或复形式),存在 $\mathscr{X}$ 上的线性泛函 f,使得 $f(x)=f_0(x)(\forall x\in E)$,且
> $$|f(x)| \leqslant p(x) = \|f_0\|_E\cdot\|x\| \quad (\forall x\in\mathscr{X}).$$
> 因此 $f\in\mathscr{X}^*$,且 $\|f\|\leqslant\|f_0\|_E$.另一方面,
> $$\|f_0\|_E = \sup_{x\in E,\|x\|=1}|f_0(x)| = \sup_{x\in E,\|x\|=1}|f(x)| \leqslant \sup_{\|x\|=1}|f(x)| = \|f\|,$$
> 故 $\|f\|=\|f_0\|_E$.$\blacksquare$
> [!corollary] 推论 2.1.19
> 设 $\mathscr{X}$ 是赋范空间,$x_0\in\mathscr{X}$, $x_0\neq\theta$.则存在 $f\in\mathscr{X}^*$,使得
> $$f(x_0) = \|x_0\|, \quad \|f\| = 1.$$
> [!proof]- 证
> 令 $E=\operatorname{span}\{x_0\}=\{\lambda x_0|\lambda\in\mathbb{K}\}$.定义 $f_0$ 为
> $$f_0(\lambda x_0) = \lambda\|x_0\| \quad (\lambda\in\mathbb{K}).$$
> 则 $f_0$ 是 E 上的线性泛函,且 $\|f_0\|_E=1$.由 Hahn-Banach 定理,存在 $f\in\mathscr{X}^*$,使得 $f(x)=f_0(x)(\forall x\in E)$,且 $\|f\|=\|f_0\|_E=1$.特别有 $f(x_0)=\|x_0\|$.$\blacksquare$
> [!corollary] 推论 2.1.20
> 设 $\mathscr{X}$ 是赋范空间,$E\subset\mathscr{X}$ 是线性子空间,$x_0\in\mathscr{X}$,且 $d\triangleq\inf_{x\in E}\|x_0-x\|>0$.则存在 $f\in\mathscr{X}^*$,使得
> $$f(x_0) = d, \quad f(x) = 0 \quad (\forall x\in E), \quad \|f\| = 1.$$
> 1.4.14 设 $C_0$ 表示以 $0$ 为极限的数列全体, 并在 $C_0$ 中赋以范数
> $$\|x\| = \sup_{n \geqslant 1} |\xi_n|, \quad \forall x = (\xi_1, \xi_2, \cdots, \xi_n, \cdots) \in C_0.$$
...
> 1.4.15 设 $\mathscr{X}$ 是 $B^*$ 空间, $M$ 是 $\mathscr{X}$ 的有限维真子空间. 求证: $\exists y \in \mathscr{X}, \|y\| = 1$, 使得
> $$\|y - x\| \geqslant 1 \quad (\forall x \in M).$$
> 1.4.16 若 $f$ 是定义在区间 $[0,1]$ 上的复值函数, 定义
> $$\omega_\delta(f) = \sup\left\{|f(x) - f(y)| \left| \forall x, y \in [0,1], |x - y| \leqslant \delta \right.\right\}.$$
> 如果 $0 < \alpha \leqslant 1$ 对应的 Lipschitz 空间 Lip $\alpha$, 由满足
> $$\|f\| \triangleq |f(0)| + \sup_{\delta > 0}\{\delta^{-\alpha}\omega_\delta(f)\} < \infty$$
> 的一切 $f$ 组成, 并以 $\|f\|$ 为范数. 又设
> $$\text{lip } \alpha \triangleq \left\{f \in \text{Lip } \alpha \left| \lim_{\delta \to 0} \delta^{-\alpha}\omega_\delta(f) = 0 \right.\right\}.$$
> 求证: Lip $\alpha$ 是 $B$ 空间, 而且 lip $\alpha$ 是 Lip $\alpha$ 的闭子空间.
> 1.4.17 设有商空间 $\mathscr{X}/\mathscr{X}_0$.
...
$$\min_{\alpha_1, \alpha_2, \cdots, \alpha_n} \sum_{j=1}^m \left|y^{(j)} - \sum_{i=1}^n \alpha_i x_i^{(j)}\right|^2 = \sum_{j=1}^m \left|y^{(j)} - \sum_{i=1}^n \lambda_i x_i^{(j)}\right|^2.$$
...
$$\min_{\alpha_1, \alpha_2, \cdots, \alpha_n} \int_a^b \left|f(x) - \sum_{i=1}^n \alpha_i \varphi_i(x)\right|^2 \mathrm{d}x$$
$$= \int_a^b \left|f(x) - \sum_{i=1}^n \lambda_i \varphi_i(x)\right|^2 \mathrm{d}x.$$
...
$$\min_{(\alpha_1, \alpha_2, \cdots, \alpha_n) \in \mathbb{R}^n} \int_\Omega \left|X(\omega) - \sum_{i=1}^n \alpha_i X_i(\omega)\right|^2 \mathrm{d}P(\omega)$$
$$= \int_\Omega \left|X(\omega) - \sum_{i=1}^n \lambda_i X_i(\omega)\right|^2 \mathrm{d}P(\omega).$$
上面三个问题本质上是一个: 在 Hilbert 空间 $\mathscr{X}$ 上给定 $x$ 及 $x_1, x_2, \cdots, x_n$, 要求出 $(\lambda_1, \lambda_2, \cdots, \lambda_n) \in \mathbb{R}^n$, 使得
$$\min_{(\alpha_1, \alpha_2, \cdots, \alpha_n) \in \mathbb{R}^n} \left\|x - \sum_{i=1}^n \alpha_i x_i\right\| = \left\|x - \sum_{i=1}^n \lambda_i x_i\right\|.$$
从几何上看, 就是求 $x$ 在由 $x_1, x_2, \cdots, x_n$ 张成的子空间 $M$ 上的正交投影. 不妨设 $\{x_1, x_2, \cdots, x_n\}$ 是线性无关的. 根据特例 1.6.36, 为了 $x_0 = \displaystyle\sum_{i=1}^n \lambda_i x_i$ 是所求的解, 必须且仅须
$$(x - x_0, x_j) = 0 \quad (j = 1, 2, \cdots, n),$$
即
$$\sum_{i=1}^n \lambda_i (x_i, x_j) = (x, x_j) \quad (j = 1, 2, \cdots, n).$$ \hfill (1.6.34)
...
**1.6.4** 设 $M, N$ 是内积空间中的两个子集, 求证:
$$M \subset N \Longrightarrow N^\perp \subset M^\perp.$$
**1.6.5** 设 $M$ 是 Hilbert 空间 $\mathscr{X}$ 的子集, 求证:
$$(M^\perp)^\perp = \overline{\operatorname{span} M}.$$
**1.6.6** 在 $L^2[-1,1]$ 中, 问偶函数集的正交补是什么? 证明你的结论.
**1.6.7** 在 $L^2[a,b]$ 中, 考察函数集 $S = \{\mathrm{e}^{2\pi\mathrm{i}nx}\}$.
(1) 若 $|b-a| \leqslant 1$, 求证: $S^\perp = \{\theta\}$;
(2) 若 $|b-a| > 1$, 求证: $S^\perp \neq \{\theta\}$.
**1.6.8** 设 $\mathscr{X}$ 表示闭单位圆上的解析函数全体, 内积定义为
$$(f, g) = \frac{1}{\mathrm{i}} \oint_{|z|=1} \frac{f(z)\overline{g(z)}}{z} \mathrm{d}z \quad (\forall f, g \in \mathscr{X}).$$
求证: $\{z^n/\sqrt{2\pi}\}$ 是一组正交规范集.
**1.6.9** 设 $\{e_n\}_{n=1}^\infty, \{f_n\}_{n=1}^\infty$ 是 Hilbert 空间 $\mathscr{X}$ 中的两个正交规范集, 满足条件
$$\sum_{n=1}^\infty \|e_n - f_n\|^2 < 1.$$
求证: $\{e_n\}$ 和 $\{f_n\}$ 两者中一个完备蕴含另一个完备.
**1.6.10** 设 $\mathscr{X}$ 是 Hilbert 空间, $\mathscr{X}_0$ 是 $\mathscr{X}$ 的闭线性子空间, $\{e_n\}, \{f_n\}$ 分别是 $\mathscr{X}_0$ 和 $\mathscr{X}_0^\perp$ 的正交规范基. 求证: $\{e_n\} \cup \{f_n\}$ 是 $\mathscr{X}$ 的正交规范基.
**1.6.11** 设 $H^2(D)$ 是按例 1.6.28 定义的内积空间.
(1) 如果 $u(z)$ 的 Taylor 展开式是 $u(z) = \displaystyle\sum_{k=0}^\infty b_k z^k$, 求证:
...
**1.6.12** 设 $\mathscr{X}$ 是内积空间, $\{e_n\}$ 是 $\mathscr{X}$ 中的正交规范集, 求证:
$$\left|\sum_{n=1}^\infty (x, e_n)\overline{(y, e_n)}\right| \leqslant \|x\| \cdot \|y\| \quad (\forall x, y \in \mathscr{X}).$$
**1.6.13** 设 $\mathscr{X}$ 是一个内积空间, $\forall x_0 \in \mathscr{X}$, $\forall r > 0$, 令
...
**1.6.14** 求 $(a_0, a_1, a_2) \in \mathbb{R}^3$, 使得 $\displaystyle\int_0^1 |\mathrm{e}^t - a_0 - a_1 t - a_2 t^2|^2 \mathrm{d}t$ 取最小值.
**1.6.15** 设 $f(x) \in C^2[a,b]$, 满足边界条件
$$f(a) = f(b) = 0, \quad f'(a) = 1, \quad f'(b) = 0.$$
求证:
$$\int_a^b |f''(x)|^2 \mathrm{d}x \geqslant \frac{4}{b-a}.$$
**1.6.16 (变分不等式)** 设 $\mathscr{X}$ 是一个 Hilbert 空间, $a(x,y)$ 是 $\mathscr{X}$ 上的共轭对称的双线性函数, $\exists M > 0, \delta > 0$, 使得
...
$$\operatorname{Re}[2a(x_0, x - x_0) - (u_0, x - x_0)] \geqslant 0 \quad (\forall x \in C).$$
# 第二章 线性算子与线性泛函
...
$$M \triangleq \bigcap_{k=1}^n N(f_k), \quad N(f_k) \triangleq \{x \in H \mid f_k(x) = 0\}$$
$$\qquad \qquad (k = 1, 2, \cdots, n).$$
...
(2) 求证: $\exists u_0 \in C$, 使得 $f(u_0) = \displaystyle\inf_{v \in C} f(v)$.
...
> [!note] 注
> 满足条件 (1) 与 (2) 的函数 $K(x,y)$ 称为 $H$ 的**再生核**.

**2.2.4** 求证: $H^2(D)$ (定义见例 1.6.28) 的再生核为
$$K(z,w) = \frac{1}{\pi(1 - z\overline{w})^2} \quad (z, w \in D).$$

**2.2.5** 设 $L, M$ 是 $H$ 上的闭线性子空间, 求证:
(1) $L \perp M \Longleftrightarrow P_L P_M = 0$;
(2) $L = M^\perp \Longleftrightarrow P_L + P_M = I$ (恒同算子);
(3) $P_L P_M = P_{L \cap M} \Longleftrightarrow P_L P_M = P_M P_L$.
...
因为由 $Tx = y$ 及 $T_l^{-1}$ 存在便推得 $x = T_l^{-1} T x = T_l^{-1} y$, 所以解 $x$ 唯一地被 $y$ 决定. 因此为了解存在而且唯一, 必须且仅须线性算子 $T$ 既有左逆又有右逆. 又因为, 如果算子 $T$ 左右逆同时存在, 那么它们一定是相等的. 事实上,
$$T_l^{-1} = T_l^{-1} I = T_l^{-1} (TT_r^{-1}) = (T_l^{-1} T) T_r^{-1} = I T_r^{-1} = T_r^{-1}.$$
...
回顾命题 1.5.10, 复线性空间 $\mathscr{X}$ 上, 只要含有一个均衡吸收凸集, 由它便可决定这空间上的一个半范数 $p(x)$. 我们试看从它能否产生 $\mathscr{X}$ 上的一个非零的连续线性泛函. 设 $x_0 \in \mathscr{X}$, 使 $p(x_0) \neq 0$. 如果我们规定:

$$\mathscr{X}_0 \triangleq \{\lambda x_0 \mid \lambda \in \mathbb{K}\}, \quad f_0(\lambda x_0) \triangleq \lambda p(x_0) \quad (\forall \lambda \in \mathbb{K}).$$

那么 $f_0$ 就是 $\mathscr{X}_0$ 上的一个非零线性泛函, 满足有界性条件:

$$|f_0(\lambda x_0)| \leqslant |\lambda p(x_0)| = p(\lambda x_0) \quad (\forall \lambda \in \mathbb{K}).$$

如果能把这个定义在 $\mathscr{X}_0$ 上的连续线性泛函延拓成整个空间 $\mathscr{X}$ 上的连续线性泛函, 问题就解决了. 下面要证明的 Hahn-Banach 定理正是保证这种延拓的可能性, 不过为了多方面的应用, 提法上稍为一般些.
...
> [!proof]-
> $\forall y_0 \in \mathscr{X} \setminus \mathscr{X}_0$, 记 $\mathscr{X}_1 \triangleq \{x + \alpha y_0 \mid x \in \mathscr{X}_0, \alpha \in \mathbb{R}\}$. 首先, 将 $f_0$ 延拓到 $\mathscr{X}_1$, 设延拓后的线性泛函记为 $f_1$, 那么
> $$f_1(x + \alpha y_0) = f_0(x) + \alpha f_1(y_0) \quad (\forall x \in \mathscr{X}_0, \forall \alpha \in \mathbb{R}). \qquad (2.4.1)$$
> 可见问题只在于决定 $f_1(y_0)$ 的值. 既然要求 $f_1$ 满足受 $p$ 控制条件, 所以
> $$f_1(x + \alpha y_0) \leqslant p(x + \alpha y_0) \quad (\forall x \in \mathscr{X}_0, \forall \alpha \in \mathbb{R}). \qquad (2.4.2)$$
> 对 (2.4.2) 式两边同除以 $|\alpha|$, 推出它等价于
> $$\begin{cases}f_1(y_0 - z) \leqslant p(y_0 - z), & \forall z \in \mathscr{X}_0, \\ f_1(-y_0 + y) \leqslant p(-y_0 + y), & \forall y \in \mathscr{X}_0,\end{cases}$$
> 或
> $$\begin{aligned}f_0(y) - p(-y_0 + y) &\leqslant f_1(y_0) \leqslant f_0(z) + p(y_0 - z) \\ &(\forall y, z \in \mathscr{X}_0).\end{aligned}$$
> 于是为了能取到适合 (2.4.2) 式的 $f_1(y_0)$ 必须且仅须:
> $$\sup_{y \in \mathscr{X}_0} \{f_0(y) - p(-y_0 + y)\} \leqslant \inf_{z \in \mathscr{X}_0} \{f_0(z) + p(y_0 - z)\}. \qquad (2.4.3)$$
...
> 所以
> $$f_0(y) - p(-y_0 + y) \leqslant f_0(z) + p(y_0 - z) \quad (\forall y, z \in \mathscr{X}_0). \qquad (2.4.4)$$
...
> $$\mathscr{F} \triangleq \left\{(\mathscr{X}_\Delta, f_\Delta) \left| \begin{array}{l}\mathscr{X}_0 \subset \mathscr{X}_\Delta \subset \mathscr{X}; \\ \forall x \in \mathscr{X}_0 \Rightarrow f_\Delta(x) = f_0(x); \\ \forall x \in \mathscr{X}_\Delta \Rightarrow f_\Delta(x) \leqslant p(x)\end{array} \right. \right\}.$$
...
> $$(\widetilde{\mathscr{X}_\Lambda}, \widetilde{f}_\Lambda) \in \mathscr{F}, \quad \text{使得 } \mathscr{X}_\Lambda \subset \widetilde{\mathscr{X}_\Lambda}, \text{ 但是 } \mathscr{X}_\Lambda \neq \widetilde{\mathscr{X}_\Lambda}.$$
...
为 $f$ 的 Fourier 级数. 当 $f \in L^2[0, 2\pi]$ 时, 级数 (2.5.35) 在 $L^2[0, 2\pi]$ 空间上收敛 (见例 1.6.26). 一般来说, 对于 $L^1[0, 2\pi]$ 中的 $f$, 级数(2.5.35)未必收敛. 通常我们考察下列 Cesàro 部分和 (算术平均和):

$$\begin{aligned} \sigma_n(f)(x) &= \frac{1}{n+1} \sum_{k=0}^{n} S_k(f)(x) \\ &= \sum_{k=-n}^{n} c_k \left(1 - \frac{|k|}{n+1}\right) e^{ikx}, \end{aligned} \qquad (2.5.36)$$

其中 $S_n(f)(x) \triangleq \displaystyle\sum_{k=-n}^{n} c_k e^{ikx}$. 通过初等计算, 有

$$\sigma_n(f)(x) = \int_0^{2\pi} f(y) K_n(x-y) dy,$$

其中

$$\begin{aligned} K_n(x) &\triangleq \frac{1}{2\pi} \sum_{k=-n}^{n} \left(1 - \frac{|k|}{n+1}\right) e^{ikx} \\ &= \frac{1}{2\pi(n+1)} \left(\frac{\sin(n+1)\frac{x}{2}}{\sin\frac{x}{2}}\right)^2, \end{aligned}$$
...
> [!proof]-
> 这是压缩映射原理 (定理 1.1.12) 的推论. 事实上, $\|(I - T)^{-1}\| \leqslant M \Longleftrightarrow \forall y \in \mathscr{X}, \exists x \in \mathscr{X}$, 使得 $x$ 是 $Sx \triangleq y + Tx$ 的不动点, 并且 $\|x\| \leqslant M\|y\|$. 如今
> $$\|Sx - Sx'\| = \|Tx - Tx'\| \leqslant \|T\| \cdot \|x - x'\|  (\forall x, x' \in \mathscr{X}),$$
> 即 $S$ 是压缩映射. 从而有唯一 $x = Sx = y + Tx$, 并且
> $$\|x\| \leqslant \frac{\|y\|}{1 - \|T\|}. 

> [!remark] 注 事实上, 我们有
> $$(I - T)^{-1} = \sum_{k=0}^{\infty} T^k, \qquad (2.6.2)$$
> 其意义是 $\forall x \in \mathscr{X}$,
> $$(I - T)^{-1} x = \sum_{k=0}^{\infty} T^k x.$$

> [!proof]-
> 当 $\|T\| < 1$ 时, $\|T^k\| \leqslant \|T\|^k \to 0 (k \to \infty)$. 因此 $\sum_{k=0}^{\infty} T^k$ 按算子范数收敛. 而 $\forall n \in \mathbb{N}$,
> $$(I - T) \sum_{k=0}^{n} T^k = \sum_{k=0}^{n} T^k - \sum_{k=1}^{n+1} T^k = I - T^{n+1},$$
> 令 $n \to \infty$, $T^{n+1} \to O$, 即得 (2.6.2) 式.
...
> [!proof]-
> (1) 设 $\mathscr{X}_n = N((I - A)^n)$, $\mathscr{Y}_n = R((I - A)^n)$, 则显然有
> $$\mathscr{X} \supset \mathscr{Y}_1 \supset \mathscr{Y}_2 \supset \cdots \supset \mathscr{Y}_n \supset \cdots,$$
> 且 $I - A : \mathscr{Y}_n \to \mathscr{Y}_{n+1}$ 诱导出自然映射
> $$\widetilde{I - A} : \mathscr{X}/\mathscr{X}_n \to \mathscr{X}/\mathscr{X}_{n+1}.$$
> 我们断言: 存在某个 $n$, 使得 $\mathscr{X}_n = \mathscr{X}_{n+1}$. 否则的话, 将有严格包含关系
> $$\mathscr{X}_1 \subsetneq \mathscr{X}_2 \subsetneq \cdots \subsetneq \mathscr{X}_n \subsetneq \cdots,$$
> 由 Riesz 引理, $\exists y_n \in \mathscr{X}_n$, $\|y_n\| = 1$, $\operatorname{dist}(y_n, \mathscr{X}_{n-1}) \geqslant 1/2$. 但这时有
> $$\|Ay_n - Ay_m\| = \|(I - A)y_n - (I - A)y_m + y_m - y_n\| \geqslant \frac{1}{2}  (m > n),$$
> 这与 $A$ 是紧算子矛盾. 因此, $\exists n_0$ 使得 $\mathscr{X}_{n_0} = \mathscr{X}_{n_0+1}$.
> 再证 $R(I - A)$ 是闭的. 设 $y_n = (I - A)x_n \to y$. 因为 $\mathscr{X} = \mathscr{X}_{n_0} \oplus M$ (某个闭子空间 $M$), 可设 $x_n = u_n + v_n$, 其中 $u_n \in \mathscr{X}_{n_0}$, $v_n \in M$. 由于 $\mathscr{X}_{n_0} = N((I - A)^{n_0})$, 故 $y_n = (I - A)v_n$. 由 $M$ 上 $(I - A)$ 是单射, 并且 $I - A : M \to R(I - A)$ 是双射. 设 $v_n \to v \in M$, 则 $y = (I - A)v \in R(I - A)$.
> (2) 在 $N(I - A)$ 的单位球上, $Ix = Ax$, 而 $A$ 是紧算子, 所以 $N(I - A)$ 中的单位球是相对紧的, 从而是紧的. 由定理 1.4.28, $N(I - A)$ 是有限维的.
> (3) 由引理 2.6.15, $\operatorname{codim} R(I - A) = \dim N(I - A^*)$. 用 $\mathscr{X}^*$ 代替 $\mathscr{X}$, 并注意 $A^*$ 也是紧算子 (见习题 2.5.7), 由 (2) 知 $\dim N(I - A^*) < \infty$.
> 再证 $\dim N(I - A) = \dim N(I - A^*)$. 假设 $\dim N(I - A) > \dim N(I - A^*)$. 由 Hahn-Banach 定理, 可以找到非零元 $x_0 \in N(I - A)$, 使得 $x_0 \notin R(I - A)$. 但这样 $\{x_0\} \oplus R(I - A)$ 就严格包含 $R(I - A)$, 矛盾. 因此
> $$\dim N(I - A) = \dim N(I - A^*).$$
> (4) 设 $\lambda \neq 0$, $\lambda \in \sigma(A)$, 且 $\lambda \notin \sigma_p(A)$. 则 $(\lambda I - A)^{-1}$ 存在, 且 $R(\lambda I - A) \neq \mathscr{X}$. 由 (1), $R(\lambda I - A)$ 是闭的. 又由 (3),
> $$\operatorname{codim} R(\lambda I - A) = \dim N(\lambda I - A) = 0,$$
> 所以 $R(\lambda I - A) = \mathscr{X}$, 即 $\lambda \in \rho(A)$, 矛盾.
...
$\|x\| = \sup_{N \in \mathbb{N}} \|S_N x\|,$
其中 $S_N x = \sum_{n=1}^{N} C_n(x) e_n (\forall x \in \mathscr{X})$. 不难验证 $\mathscr{X}$ 按 $\square \cdot \square$ 是完备的, 并且
$\|x\| = \lim_{N \to \infty} \|S_N x\| \leqslant \square x \square  (\forall x \in \mathscr{X}).$
由等价范数定理 (推论 2.3.14) $\exists M_1 > 0$, 使得
$\square x \square \leqslant M_1 \|x\|  (\forall x \in \mathscr{X}).$
于是对 $\forall n \in \mathbb{N}$, 我们有
$\|C_n(x) e_n\| = \|S_n x - S_{n-1} x\| \leqslant 2\square x \square \leqslant 2M_1 \|x\|  (\forall x \in \mathscr{X}).$
由此可见, $\forall n \in \mathbb{N}$,
$|C_n(x)| \leqslant 2M_1 \|e_n\|^{-1} \cdot \|x\|  (\forall x \in \mathscr{X}).$
故 $C_n(x)$ 是连续的 $(n = 1, 2, \cdots)$. 
Banach 在 1932 年提出一个问题: 是否每个可分的 Banach 空间都具有 Schauder 基? 如果这个结论是对的, 那么便能推出 $\overline{F(\mathscr{X})} = \mathfrak{C}(\mathscr{X})$. 然而到了 1973 年, Enflo 做出了否定的回答: 存在一个可分的 Banach 空间和其上的一个紧算子, 这个紧算子不能被有穷秩算子所逼近.^① 不久, Davie 给出了一个较简单的证明.^②
## 习			题
...
> $\mathrm{dist}(y_k, \mathscr{X}_{k+1}) \geqslant \frac{1}{2}  (k = 0, 1, 2, \cdots).$
...
> 这与 $x_{n+1} \neq \theta$ 矛盾. 因此 $\{x_1, x_2, \cdots, x_{n+1}\}$ 是线性无关的.
> $2^\circ$  若令 $E_n = \mathrm{span}\{x_1, x_2, \cdots, x_n\}$, 则 $E_n \subsetneqq E_{n+1}$, 应用 Riesz 引理 (引理 1.4.31), $\exists y_{n+1} \in E_{n+1}$, 使得
> $\|y_{n+1}\| = 1,  \text{且}  \mathrm{dist}(y_{n+1}, E_n) \geqslant \frac{1}{2}.$
> 从而对 $\forall n, p \in \mathbb{N}$, 有
> $\begin{aligned}$
> $&\left\|\frac{1}{\lambda_{n+p}} Ay_{n+p} - \frac{1}{\lambda_n} Ay_n\right\| \\$
> $&= \left\|y_{n+p} - \left(y_{n+p} - \frac{1}{\lambda_{n+p}} Ay_{n+p} + \frac{1}{\lambda_n} Ay_n\right)\right\| \geqslant \frac{1}{2}.$
> $\end{aligned}$
> 这是因为
...
> (3) $\sigma(A) = \{\lambda_1, \lambda_2, \cdots, \lambda_n, \cdots\}$, 其中 $\lambda_n \to 0$.
> 试举例说明: 这三种情形都可能发生.
...
> $\begin{aligned}$
> $x &= x_M + x_{M^\perp}  (x_M \in M, x_{M^\perp} \in M^\perp), \\$
> $y &= y_M + y_{M^\perp}  (y_M \in M, y_{M^\perp} \in M^\perp).$
> $\end{aligned}$
...
> $A \text{ 对称} \Longleftrightarrow a(x, y) = \overline{a(y, x)}  (\forall x, y \in H). \tag{3.4.3}$
> 又由命题 1.6.2, 我们有
...
$\begin{aligned}$
$\|(\lambda I - A)^{-1} x\| &\leqslant \frac{1}{|\mathrm{Im} \lambda|} \|x\| \\$
$(\forall x \in H, &\forall \lambda \in \mathbb{C}, \mathrm{Im} \lambda \neq 0).$
$\end{aligned}$
> [!proof]-
> 设 $\lambda = \mu + \mathrm{i}\nu, \nu \neq 0, \mu, \nu \in \mathbb{R}$, 则由对称性,
> $\begin{aligned}$
> $\|(\lambda I - A)x\|^2 &= \|(\mu I - A)x\|^2 + |\nu|^2 \cdot \|x\|^2 \\$
> $&\geqslant |\nu|^2 \cdot \|x\|^2  (\forall x \in H). \tag{3.4.5}$
> $\end{aligned}$
> 此外, $R(\lambda I - A) = H$, 这是因为:
> $R(\lambda I - A)^\perp = N(\overline{\lambda} I - A^*) = N(\overline{\lambda} I - A),$
> 再由 (3.4.5) 式, $N(\overline{\lambda} I - A) = \{\theta\}$ (当 $\mathrm{Im} \lambda \neq 0$). 
...
$\sup_{\|x\| = 1} |(Ax, x)| = \|A\|.$
> [!proof]-
> 记 $C \triangleq \sup_{\|x\| = 1} |(Ax, x)|$, $c \triangleq \max\{|m|, |M|\}$, 这里
> $m \triangleq \inf_{\|x\| = 1} (Ax, x),  M \triangleq \sup_{\|x\| = 1} (Ax, x).$

> [!proof]-
> $c \leqslant C$ 是显然的. 下证 $C \leqslant c$. 由 $A$ 的对称性和平行四边形法则, 有
> $\mathrm{Re}(Ax, y) = \frac{1}{4} [(A(x + y), x + y) - (A(x - y), x - y)]$
> $\leqslant \frac{c}{4} [\|x + y\|^2 + \|x - y\|^2] = \frac{c}{2} (\|x\|^2 + \|y\|^2). \tag{3.4.6}$
> 取 $e^{\mathrm{i}\theta}$ 使得
> $\mathrm{Re}(Ax, y) e^{\mathrm{i}\theta} = (Ax, y),$
> 即
> $\mathrm{Re}(Ax, e^{\mathrm{i}\theta} y) = (Ax, y).$
> 于是对 $\forall x, y \in H$, 以及 $\forall \theta \in [0, 2\pi]$, 由 (3.4.6) 式,
> $|(Ax, y)| = \mathrm{Re}(Ax, e^{\mathrm{i}\theta} y) \leqslant \frac{c}{2} (\|x\|^2 + \|y\|^2). \tag{3.4.7}$
> 取 $y = \dfrac{\|x\|}{\|Ax\|} Ax (\forall x \neq \theta)$, 就有
> $\|Ax\| \leqslant c \|x\|  (\forall x \in H).$
> 因此 $\|A\| \leqslant c = \max\{|m|, |M|\}$. 
...
$$\Box u\Box=\left[\int_\Omega|\nabla u|^2\mathrm{d}x+\int_\Omega(U(x)+\lambda_0)u^2(x)\mathrm{d}x\right]^{\frac{1}{2}},$$
其中 $\lambda_0\triangleq\max_{x\in\overline{\Omega}}|U(x)|$. 根据 Poincar\'e 不等式 (引理 1.6.15), 存在常数 $m,M>0$, 使得
$$m\|u\|_1 \leqslant \Box u\Box \leqslant M\|u\|_1 \quad (\forall u\in H_0^1(\Omega)), \qquad (3.5.3)$$
其中 $\|\cdot\|_1$ 表示 $H_0^1(\Omega)$ 的范数. (3.5.3) 式表明 $\Box\cdot\Box$ 是 $H_0^1(\Omega)$ 的等价范数, 所以 $H_0^1(\Omega)$ 在范数 $\Box\cdot\Box$ 下是 Banach 空间. 又令
$$(u,v)_{\lambda_0}=\int_\Omega\nabla u\cdot\nabla v\mathrm{d}x+\int_\Omega(U(x)+\lambda_0)u(x)v(x)\mathrm{d}x, \qquad (3.5.4)$$
其中 $u,v\in H_0^1(\Omega)$. $(\cdot,\cdot)_{\lambda_0}$ 是由范数 $\Box\cdot\Box$ 产生的内积. $H_0^1(\Omega)$ 在内积 (3.5.4) 式下是 Hilbert 空间. 记 $\|\cdot\|$ 为 $L^2(\Omega)$ 的范数, 再由 Poincar\'e 不等式 (引理 1.6.15), 对 $\forall u\in L^2(\Omega)$, 我们有
$$\left|\int_\Omega u\cdot v\mathrm{d}x\right| \leqslant \|u\|\cdot\|v\| \leqslant c\|u\|\Box v\Box \quad (\forall v\in H_0^1(\Omega)), \qquad (3.5.5)$$
其中 $c$ 为正常数. 于是按 Riesz 表示定理 (定理 2.2.1), $\exists!\ w\in H_0^1(\Omega)$, 使得
$$\int_\Omega uv\mathrm{d}x=(w,v)_{\lambda_0} \quad (\forall v\in H_0^1(\Omega)). \qquad (3.5.6)$$
定义 $K_{\lambda_0}: L^2(\Omega)\to H_0^1(\Omega)$ 为 $w=K_{\lambda_0}u$, 便有
$$\Box K_{\lambda_0}u\Box \leqslant c\|u\| \quad (\forall u\in L^2(\Omega)).$$
因此 $K_{\lambda_0}$ 是 $L^2(\Omega)\to H_0^1(\Omega)$ 的连续线性算子.

记 $\iota$ 为 $H_0^1(\Omega)\to L^2(\Omega)$ 的嵌入算子, 由 Rellich 定理 (定理 4.5.10) 的推论 4.5.11, $\iota$ 是紧的. 于是 (3.5.2) 式等价于
$$(u,v)_{\lambda_0}=(K_{\lambda_0}f,v)_{\lambda_0}+\lambda_0(K_{\lambda_0}\iota u,v)_{\lambda_0} \quad (\forall v\in H_0^1(\Omega)),$$
即等价于在 $H_0^1(\Omega)$ 中解方程
$$(I-\lambda_0 K_{\lambda_0}\iota)u=K_{\lambda_0}f. \qquad (3.5.7)$$
应用 Riesz-Fredholm 定理 (定理 3.2.1),
$$\text{方程 (3.5.7) 有解} \Longleftrightarrow K_{\lambda_0}f \perp N(I-\lambda_0 K_{\lambda_0}\iota).$$
然而
$$\begin{aligned}
u\in N(I-\lambda_0 K_{\lambda_0}\iota) &\Longleftrightarrow u=\lambda_0 K_{\lambda_0}\iota u \\
&\Longleftrightarrow (u,v)_{\lambda_0}=\lambda_0\int_\Omega uv\mathrm{d}x \quad (\forall v\in H_0^1(\Omega)) \\
&\Longleftrightarrow \int_\Omega(\nabla u\cdot\nabla v+U(x)uv)\mathrm{d}x=0 \quad (\forall v\in H_0^1(\Omega)),
\end{aligned}$$
即 $u$ 是方程 (3.5.1) 的齐次方程的弱解. 设方程 (3.5.1) 的齐次方程的弱解由 $\{\varphi_1,\varphi_2,\cdots,\varphi_n\}$ 张成, 那么
$$(K_{\lambda_0}f,\varphi_i)_{\lambda_0}=0 \Longleftrightarrow \int_\Omega f\varphi_i\mathrm{d}x=0 \quad (i=1,2,\cdots,n).$$
总结起来, 我们得到如下定理.
...
4.1.4 在 $\mathscr{D}'(\mathbb{R})$ 中, 求证:
(1) $\displaystyle \frac{1}{\pi} \cdot \frac{1}{x^2 + \varepsilon^2} \to \delta(x) \quad (\varepsilon \to 0+)$;
(2) $\displaystyle \frac{1}{2\sqrt{\pi t}} \exp\left(-\frac{x^2}{4t}\right) \to \delta(x) \quad (t \to 0+)$.

4.1.5 设 $\Omega \subset \mathbb{R}^n$ 是一个开集, 又设 $K$ 是 $\Omega$ 的一个紧子集, 求证: 存在一个函数 $\varphi \in C_0^{\infty}(\Omega)$, 使得 $0 \leqslant \varphi(x) \leqslant 1$, 且 $\varphi(x)$ 在 $K$ 的一个邻域内恒为 $1$.
...
$$\mathscr{D}_K = \{\varphi \in C^{\infty}(\Omega) | \mathrm{supp}(\varphi) \subset K\},$$
...
$$\|\varphi\|_m = \sum_{|\alpha| \leqslant m} \max_{x \in K} |\partial^{\alpha}\varphi(x)| \quad (m = 1, 2, \cdots). \qquad (4.2.1)$$
...
证明从略, 留作习题 (参照 $S$ 空间的收敛性, 见例 1.4.7).
...
4.2.3 验证: 空间 $S$ (例 1.4.7) 是完备空间.

4.2.4 设 $\{x_n\}$ 是 $B_0^*$ 空间 $\mathscr{X}$ 中满足 $\sum_{n=1}^{\infty} \|x_n\|_m < \infty (\forall m)$ 的序列. 求证: $\sum_{n=1}^{\infty} x_n$ 收敛.
...
记 $\mathfrak{S}(\mathscr{X}, \mathscr{Y}) = \{A \in \mathscr{L}(\mathscr{X}, \mathscr{Y}) \mid A \text{ 是紧算子}\}$, 当 $\mathscr{X} = \mathscr{Y}$ 时, 简记为 $\mathfrak{S}(\mathscr{X})$.

> [!proposition] 命题 5.1.2
> (1) 若 $A, B \in \mathfrak{S}(\mathscr{X}, \mathscr{Y})$, $\alpha, \beta \in \mathbb{R}$, 则 $\alpha A + \beta B \in \mathfrak{S}(\mathscr{X}, \mathscr{Y})$.
>
> (2) 若 $A_n \in \mathfrak{S}(\mathscr{X}, \mathscr{Y})$, $A \in \mathscr{L}(\mathscr{X}, \mathscr{Y})$, 且 $\|A_n - A\| \to 0$, 则 $A \in \mathfrak{S}(\mathscr{X}, \mathscr{Y})$. 即 $\mathfrak{S}(\mathscr{X}, \mathscr{Y})$ 是 $\mathscr{L}(\mathscr{X}, \mathscr{Y})$ 的闭子空间.
>
> (3) 若 $A \in \mathfrak{S}(\mathscr{X}, \mathscr{Y})$, $T_1 \in \mathscr{L}(\mathscr{W}, \mathscr{X})$, $T_2 \in \mathscr{L}(\mathscr{Y}, \mathscr{Z})$, 则 $T_2AT_1 \in \mathfrak{S}(\mathscr{W}, \mathscr{Z})$.
...
> [!remark] 注 1
> $\mathfrak{S}(\mathscr{X}, \mathscr{Y})$ 是 $\mathscr{L}(\mathscr{X}, \mathscr{Y})$ 的一个闭子空间, 因此 $\mathfrak{S}(\mathscr{X}, \mathscr{Y})$ 本身也是一个 Banach 空间.
...
> [!proof]-
> 要证: 由 $\{u_m\}_{m=1}^\infty \subset W^{1,2}(\varOmega)$, $\|u_m\|_{W^{1,2}} \leqslant 1$, 可抽出子列 $\{u_{m'}\}$, 使得 $\{u_{m'}\}$ 在 $L^2(\varOmega)$ 中收敛. 为此记 $U_m = Tu_m$, 其中 $T$ 是扩张算子, 并且由习题 4.5.2, 不妨设 $U_m$ 在公共的、在 $\mathbb{R}^n$ 中紧的集合 $K$ 外为 0. 我们要证
> $$\|u_{p'}-u_{m'}\|_{L^2(\varOmega)} \to 0 \quad (\text{当 } m', p' \to \infty).$$
> 注意到
> $$\|u_{p'}-u_{m'}\|_{L^2(\varOmega)}^2 \leqslant C\|U_{p'}-U_{m'}\|_{L^2(\mathbb{R}^n)}^2$$
> $$= C\int_{\mathbb{R}^n}|\widehat{U}_{p'}(\xi)-\widehat{U}_{m'}(\xi)|^2\,\mathrm{d}\xi,$$
> 而
> $$\widehat{U}_m(\xi) = \int_K U_m(x)\mathrm{e}^{-2\pi\mathrm{i}x\cdot\xi}\,\mathrm{d}x$$
> $$= \langle\mathrm{e}^{-2\pi\mathrm{i}x\cdot\xi}\alpha_k, U_m\rangle, \qquad (4.5.7)$$
> 其中
> $$\alpha_k(x) = \begin{cases} 1, & x \in K, \\ 0, & x \overline{\in} K. \end{cases}$$
> 由于 $\|U_m\|_{L^2(\mathbb{R}^n)} \leqslant C$ 以及 $\alpha_k\mathrm{e}^{-2\pi\mathrm{i}x\cdot\xi} \in L^2(\mathbb{R}^n)$, 再根据 Hilbert 空间 $L^2(\mathbb{R}^n)$ 的自反性和 Eberlein-Smulian 定理 (定理 2.5.28), 有子列 $\{m'\}$, 使得 $\{U_{m'}\}$ 弱收敛. 从而 (4.5.7) 式蕴含了对 $\forall\xi \in \mathbb{R}^n$, $\widehat{U}_{m'}(\xi)$ 收敛. 又
> $$|\widehat{U}_m(\xi)| \leqslant \left(\int_K\mathrm{d}x\right)^{\frac{1}{2}}\left(\int_K|U_m(x)|^2\,\mathrm{d}x\right)^{\frac{1}{2}}$$
> $$\leqslant [\mathrm{mes}(K)]^{\frac{1}{2}}\|U_m\|_{L^2} \leqslant \mathrm{const}.$$
> 这时 $\forall r > 0$,
> $$\int_{\mathbb{R}^n}|\widehat{U}_{m'}(\xi)-\widehat{U}_{p'}(\xi)|^2\,\mathrm{d}\xi = \int_{|\xi|\leqslant r} + \int_{|\xi|\geqslant r} = \mathrm{I} + \mathrm{II}.$$
> 然而
> $$\mathrm{II} \leqslant r^{-2}\int_{|\xi|\geqslant r}(1+|\xi|^2)|\widehat{U}_{m'}(\xi)-\widehat{U}_{p'}(\xi)|^2\,\mathrm{d}\xi$$
> $$\leqslant 2r^{-2}\int_{|\xi|\geqslant r}(1+|\xi|^2)(|\widehat{U}_{m'}(\xi)|^2+|\widehat{U}_{p'}(\xi)|^2)\,\mathrm{d}\xi$$
> $$\leqslant 2r^{-2}(\|U_{m'}\|_1'^2 + \|U_{p'}\|_1'^2) \leqslant Mr^{-2},$$
> 其中 $M$ 是一个仅依赖于扩张算子 $T$ 的常数.
> $\forall\varepsilon > 0$, 取 $r$ 足够大, 使 $Mr^{-2} < \varepsilon/2$, 固定 $r$, 根据 Lebesgue 控制收敛定理, 当 $m', p'$ 足够大时,
> $$\mathrm{I} = \int_{|\xi|\leqslant r}|\widehat{U}_{m'}(\xi)-\widehat{U}_{p'}(\xi)|^2\,\mathrm{d}\xi < \frac{\varepsilon}{2},$$
> 即得
> $$\|u_{m'}-u_{p'}\|_{L^2(\varOmega)} \to 0 \quad (\text{当 } m', p' \to \infty). \qquad \blacksquare$
...
> [!proof]-
> 充分性显然, 只需证必要性. 设 $f \in H^{-m}(\varOmega)$, 因为 $H_0^m(\varOmega)$ 是 Hilbert 空间, 应用 Riesz 表示定理 (定理 2.5.4), $f$ 对应着一个 $h \in H_0^m(\varOmega)$, 使得
> $$\langle f, \varphi \rangle = (\varphi, h)_m = \sum_{|\alpha|\leqslant m}\int_\varOmega \overline{\partial^\alpha h(x)} \cdot \partial^\alpha\varphi(x)\,\mathrm{d}x$$
> $$= \sum_{|\alpha|\leqslant m}\int_\varOmega g_\alpha(x)\partial^\alpha\varphi(x)\,\mathrm{d}x \quad (\forall \varphi \in H_0^m(\varOmega)), \qquad (4.5.8)$$
> 其中 $g_\alpha(x) \triangleq \overline{\partial^\alpha h(x)} \in L^2(\varOmega)$. $\blacksquare$
...
2.3.6 令 $\|x\|_1 \triangleq \|x\| + \sup_{\alpha\in S^1}p(\alpha x)$, 其中 $S^1$ 是复平面上的单位圆周. 先证 $\|x\|_1$ 是 $X$ 上的完备范数, 再用等价范数定理 (推论 2.3.14).

2.3.11 设 $N(A) = \{x \in \mathscr{X} \mid Ax = 0\}$, 考虑映射 $\widetilde{A}: \mathscr{X}/N(A) \to \mathscr{Y}$, $\forall [x] \in \mathscr{X}/N(A)$, $\widetilde{A}[x] = Ax$, $\forall x \in [x]$. 证明 $\widetilde{A}$ 是单射、满射. 再由
$$\|\widetilde{A}[x]\|_{\mathscr{Y}} = \|Ax'\|_{\mathscr{Y}} \leqslant \|A\| \cdot \|x'\| \leqslant 2\|A\| \cdot \|[x]\|,$$
推出 $\widetilde{A}$ 有界. 由 Banach 逆算子定理 (定理 2.3.8), $\widetilde{A}^{-1} \in \mathscr{L}(\mathscr{Y},\mathscr{X}/N(A))$. 不妨假设 $y_0 = 0$, $y_n \to 0$, 记 $[x_n] = \widetilde{A}^{-1}y_n$, 则有
$$\|[x_n]\| = \|\widetilde{A}^{-1}y_n\| \leqslant \|\widetilde{A}^{-1}\| \cdot \|y_n\|.$$
于是, 取 $x_n \in [x_n]$, 使得 $\|x_n\| \leqslant 2\|[x_n]\|$, 便有 $\|x_n\| \leqslant C\|y_n\|$, 其中 $C = 2\|\widetilde{A}^{-1}\|$.

2.3.12 (3) 注意到 $\mathscr{X}/N(T)$ 是 $B$ 空间. 考虑 $\widetilde{T}: \mathscr{X}/N(T) \to \mathscr{Y}$, $\widetilde{T}[x] = Tx$,
$$D(\widetilde{T}) = \{[x] \in \mathscr{X}/N(T) \mid x \in D(T)\},$$
显然, $N(\widetilde{T}) = [\theta]$, $R(\widetilde{T}) = R(T)$.

只要证明 $\widetilde{T}$ 是闭算子, 用 (2) 的结果, 便有
$$\|[x]\|_0 \leqslant \alpha\|\widetilde{T}[x]\|,$$
即 $d(x,N(T)) \leqslant \alpha\|Tx\|$.

2.4.4 $x_n \in \mathscr{X} \subset \mathscr{X}^{**} = \mathscr{L}(\mathscr{X}^*,\mathbb{K})$,
$$\|x_n\|_{\mathscr{X}^{**}} = \|x_n\|_{\mathscr{X}}, \quad \langle x_n,f \rangle \triangleq \langle f,x_n \rangle,$$
$$\sup_n|\langle x_n,f \rangle| = \sup_n|\langle f,x_n \rangle|$$
$$= \sup_n|f(x_n)| < \infty \quad (\forall f \in \mathscr{X}^*).$$
由共鸣定理 (定理 2.3.16) $\|x_n\|_{\mathscr{X}^{**}} \leqslant M$, 即得 $\|x_n\|_{\mathscr{X}} = \|x_n\|_{\mathscr{X}^{**}} \leqslant M$.

2.4.12 用反证法. 假定 $x_2 \in C$, 记 $\lambda = \frac{1}{m}$, 则 $x_1 = \lambda x_2 + (1-\lambda)x_0$. 只要能找到一个 $d > 0$, 使得 $B(x_1,d) \subset C$, 便与 $x_1 \in \partial C$ 的假设矛盾.

因为 $x_0 \in \overset{\circ}{C}$, 所以 $\exists\delta > 0$, 使得 $B(x_0,\delta) \subset C$. 这样, $\forall y \in B(x_0,\delta)$, 都有
$$z = \lambda x_2 + (1-\lambda)y \in C,$$
$$\begin{cases} x_1 = \lambda x_2 + (1-\lambda)x_0, \\ z = \lambda x_2 + (1-\lambda)y \end{cases} \Longrightarrow z - x_1 = (1-\lambda)(y-x_0)$$
$$\Longrightarrow \|z-x_1\| = (1-\lambda)\|y-x_0\| < (1-\lambda)\delta.$$
由此可见, 只要取 $d = (1-\lambda)\delta$ 即可. 事实上, 当 $\|z-x_1\| < d$, 取辅助点
$$y = x_0 + \frac{z-x_1}{1-\lambda} \in B(x_0,\delta) \subset C,$$
从而
$$z = x_1 + (1-\lambda)(y-x_0)$$
$$= \lambda x_2 + (1-\lambda)x_0 + (1-\lambda)(y-x_0)$$
$$= \lambda x_2 + (1-\lambda)y \in C.$$

2.4.13 注意到条件 $M$ 是闭凸集与有内点的凸集 $B(x,d)$ 是可分离的. 即根据凸集分离定理 (定理 2.4.16), 存在 $f \in \mathscr{X}^*$, $\alpha \in \mathbb{R}$, 使得对 $\forall y \in M$, $z \in B(x,d)$, 成立 $f(y) \leqslant \alpha \leqslant f(z)$. 于是
$$\sup_{y\in M}f(y) \leqslant \inf_{z\in B(x,d)}f(z) = \inf_{y\in B(\theta,1)}f(x-d(x)y)$$
$$= \inf_{y\in B(\theta,1)}[f(x)-d(x)f(y)] = f(x) - d(x)\sup_{y\in B(\theta,1)}f(y)$$
$$= f(x) - d(x)\|f\|.$$
由此可见, 取 $f_1 = \frac{f}{\|f\|}$ 即为所求.

2.5.17 注意到 $\frac{\mathrm{e}^n}{2}\int_{|x|>n}\mathrm{e}^{-|x|}\,\mathrm{d}x = 1$, 令
$$u_n(x) \triangleq \chi_{|x|>n}(x)\left(\frac{\mathrm{e}^{n-|x|}}{2}\right)^{\frac{1}{p}},$$
则
$$\|u_n\|_p = \left(\int_\mathbb{R}|u_n(x)|^p\,\mathrm{d}x\right)^{\frac{1}{p}}$$
$$= \left(\frac{\mathrm{e}^n}{2}\int_{|x|>n}\mathrm{e}^{-|x|}\,\mathrm{d}x\right)^{\frac{1}{p}} = 1,$$
$$\|I-S_n\| \geqslant \|u_n\|_p = 1.$$

2.5.24 设 $d = \inf_{x\in M}\{\|x\|\}$, $\forall n$, $\exists x_n \in M$, 使得 $d \leqslant \|x_n\| < d + \frac{1}{n} \leqslant d+1$, $\{x_n\}$ 有界. 根据定理 2.5.28, 存在 $x_{n_k} \to x_0$. 对此 $x_0$, 根据推论 2.4.6, $\exists f \in \mathscr{X}^*$, 使得 $\|f\| = 1$, $f(x_0) = \|x_0\|$. 于是, 一方面
$$x_0 \in M \Longrightarrow \|x_0\| \geqslant d,$$
另一方面
$$\|x_0\| = f(x_0) = \lim_{k\to\infty}f(x_{n_k}) \leqslant \varliminf_{k\to\infty}\|f\| \cdot \|x_{n_k}\| = d,$$
故有
$$\|x_0\| = d = \inf_{x\in M}\{\|x\|\}.$$
...
$S \triangleq \overline{B_\mathscr{X}}(0,1) \Longrightarrow S+N(K)$ 是 $\mathscr{X}/N(K)$ 中的单位球, 且 $\widetilde{K}(S+N(K)) = K(S)$ 是列紧的, 从而 $\widetilde{K}$ 是紧算子. 又 $\widetilde{K}$ 是单射连续算子 $\Longrightarrow \widetilde{K}^{-1}$ 闭, 且
$$D(\widetilde{K}^{-1}) = R(K) \stackrel{\text{条件}}{\supset} R(A).$$
从而 $\widetilde{K}^{-1}A: \mathscr{X} \to \mathscr{X}/N(K)$ 是闭算子, 且定义域是全空间. 根据闭图像定理 (定理 2.3.15), $\widetilde{K}^{-1}A$ 有界, 故有
$$A = \underbrace{\widetilde{K}}_{\text{紧}}\underbrace{(\widetilde{K}^{-1}A)}_{\text{有界}} \in \mathfrak{C}(\mathscr{X},\mathscr{Y}).$$

3.1.11 令 $v = \frac{x}{\|x\|_\mathscr{X}}$, 则 $\|v\|_\mathscr{X} = 1$, 要证的结论可改述为: 对 $\forall\varepsilon > 0$, 存在 $c(\varepsilon)$, 使得
$$\|y\|_\mathscr{Y} \leqslant \varepsilon + c(\varepsilon)\|v\|_\mathscr{Z} \quad (\forall v \in B_\mathscr{X}(\theta,1)).$$
用反证法. 如果 $\exists\varepsilon_0 > 0$, 对 $\forall n \in \mathbb{N}$, $\exists v_n \in B_\mathscr{X}(\theta,1)$ 使得 $\|v_n\|_\mathscr{Y} > \varepsilon_0 + n\|v_n\|_\mathscr{Z}$, 那么一方面, $\|v_n\|_\mathscr{Y} > \varepsilon_0$, 另一方面, $v_n \stackrel{\mathscr{Z}}{\longrightarrow} 0$ ($n \to \infty$). 但是从 $\|v_n\|_\mathscr{X} = 1$ 和 $\mathscr{X} \hookrightarrow \mathscr{Y}$ 紧, 可推出 $\exists v_{n_k} \stackrel{\mathscr{Y}}{\longrightarrow} v$, 再从 $\mathscr{Y} \hookrightarrow \mathscr{Z}$ 连续, 便有 $v_{n_k} \stackrel{\mathscr{Z}}{\longrightarrow} v$. 这样, 一方面, 我们联合 $v_n \stackrel{\mathscr{Z}}{\longrightarrow} 0$ 和 $v_{n_k} \stackrel{\mathscr{Z}}{\longrightarrow} v$ 得到 $v = 0$, 另一方面, $\|v_n\|_\mathscr{Y} > \varepsilon_0 \Longrightarrow \|v\|_\mathscr{Y} \geqslant \varepsilon_0$, 即引出矛盾.

3.6.3 (1) 用 $\tau$ 表示 $\mathscr{X} \to \mathscr{Y}$ 的嵌入算子, 那么
$$\|x\|_\mathscr{X} \leqslant c(\|x\|_\mathscr{Y} + \|Tx\|_\mathscr{Y})$$
$$\Longrightarrow \|x\|_\mathscr{X} \leqslant c(\|\tau x\|_\mathscr{Y} + \|Tx\|_\mathscr{Y})$$
$$\Longrightarrow \|x\|_\mathscr{X} \leqslant c\|\tau x\|_\mathscr{Y}, \quad \forall x \in N(T).$$
特别对于 $x_n \in N(T)$, $\|x_n\| = 1$, 有 $\{\tau x_n\}$ 有收敛子列, 不妨仍记作 $\{\tau x_n\}$,
$$\|x_n - x_m\|_\mathscr{X} \leqslant c\|\tau x_n - \tau x_m\|_\mathscr{Y} \Longrightarrow \{x_n\} \text{ 收敛},$$
从而 $N(T)$ 上的单位球列紧, 故 $\dim N(T) < \infty$.

(2) 令 $\widetilde{T}[x] \triangleq Tx$, $\forall [x] \in \mathscr{X}/N(T)$, 则 $R(T) = R(\widetilde{T})$. 为了证明 $R(\widetilde{T})$ 闭, 由习题 2.3.4 (4), 只要证 $\widetilde{T}^{-1}$ 连续.

用反证法. 如果 $\widetilde{T}^{-1}$ 不连续, 则 $\exists \|[x_n]\| = 1$, 使得 $Tx_n \to 0$.
$$\|[x_n]\| = 1 \Longrightarrow \|x_n\| < 2 \Longrightarrow \{\tau x_n\} \text{ 有收敛子列},$$
不妨假设这子列就是全体. 这样 $Tx_n$ 与 $\tau x_n$ 都是基本列, 并且
$$\|x_n\|_\mathscr{X} \leqslant c(\|\tau x_n\|_\mathscr{Y} + \|Tx_n\|_\mathscr{Y}),$$
于是有 $\|x_n - x_m\|_\mathscr{X} \to 0$.
...
以及
$$\|[x_n]\| = \|[x_n] - [x_0]\| \leqslant \|x_n - x_0\| \to 0,$$
这样与 $\|[x_n]\| = 1$ 矛盾.
...
- 稠密 12
- 次线性泛函 38
- $C^k(\overline{\varOmega})$ 34
- $C_0^m(\varOmega)$ 66
- $C_0^\infty(\varOmega)$ 252
- Cantor 集 103
- Caratheodory 定理 60
...
- Dirichlet 边值问题 95
- $\delta$ 函数 256
- $\mathscr{D}(\varOmega)$ 255
- $\mathscr{D}'(\varOmega)$ 256
...
- 列紧 16
- 零链长 218
- $L_{\mathrm{loc}}^1(\varOmega)$ 256
- $L^p(\varOmega,\mu)$ ($1 \leqslant p < \infty$) 30
- $L^\infty(\varOmega,\mu)$ 31
- $l^p$ ($1 \leqslant p < \infty$) 31
- $l^\infty$ 33
- Lagrange 乘子 138
- Laplace 方程 95
- Lax-Milgram 定理 116
