...
> 定义算子$F_{i,j}$
> 
> $$ \left(F_{ij}\left(x\right)\right)_{n}= \begin{cases} \sum _{k= 1}^{i}a_{nk}\xi _{k},& n \le j\\0,& n> j\end{cases}  $$ 
> 
> 则
> $$ \left(T\left(x\right)\right)_{n}= \sum _{k = 1}^{\infty}a_{nk}\xi _{k} $$ 
> $$ \left(\left(T-F_{ij}\right)\left(x\right)\right)_{n}= \begin{cases} \sum _{k = i+ 1}^{\infty}a_{nk}\xi _{k},& n\le j\\ \sum _{k = 1}^{\infty}a_{nk}\xi _{k},&n> j \end{cases}  $$ 
> 于是
> $$ \begin{aligned} \left\| \left(T-F_{ij}\right)\left(x\right) \right\|_{\ell ^{2}} ^{2}&= \sum _{n = 1}^{j}\left| \sum _{k = i+ 1}^{\infty}a_{nk}\xi _{k} \right|^{2}+ \sum _{n =j + 1}^{\infty}\left| \sum _{k = 1}^{\infty}a_{nk}\xi _{k} \right|^{2}\\&\le \sum _{n = 1}^{j}\left(\sum _{k = i+ 1}^{\infty}\left| a_{nk} \right|^{2} \right)\left(\sum _{k = i+ 1}^{\infty}\left| \xi _{k} \right|^{2} \right)+ \sum _{n = j + 1}^{\infty}\left(\sum _{k = 1}^{\infty}\left| a_{nk} \right|^{2} \right)\left(\sum _{k = 1}^{\infty}\left| \xi _{k} \right|^{2} \right)\\&\le \left\| x \right\|_{\ell ^{2}}^{2}\left(\sum _{n = 1}^{j}\sum _{k = i+ 1}^{\infty}\left| a_{nk} \right|^{2} + \sum _{n = j + 1}^{\infty}\sum _{k = 1}^{\infty}\left| a_{nk} \right|^{2} \right) \end{aligned}  $$ 
> 因此
> $$ \left\| \left(T-F_{ij}\right) \right\|\le \left(\sum _{n = 1}^{j}\sum _{k = i+ 1}^{\infty}\left| a_{nk} \right|^{2}+ \sum _{n = j  + 1}^{\infty}\sum _{k = 1}^{\infty}\left| a_{nk} \right|^{2}\right)^{\frac{1}{2}}   $$ 
> 由于
...
> 因此存在$j_{\varepsilon }$, 使得
> $$ \sum _{n =  j_{\varepsilon } + 1}^{\infty}\sum _{k = 1}^{\infty}\left| a_{nk} \right|^{2} < \varepsilon  $$ 
> 对于固定的$j_{\varepsilon }$, 我们有
> $$ \lim_{i\to \infty}\sum _{n = 1}^{j}\sum _{k = i+ 1}^{\infty}\left| a_{nk} \right|^{2}= 0  $$ 
> 于是存在$i_{\varepsilon }$, 使得
> $$ \sum _{n = 1}^{j}\sum _{k =  i_{\varepsilon }+ 1}^{\infty}\left| a_{nk} \right|^{2}< \varepsilon   $$ 
> 于是对于这组$i_{\varepsilon }, j_{\varepsilon }$, 
> $$ \left\| \left(T-F_{i_{\varepsilon }j_{\varepsilon }}\right) \right\|\le \left(2\varepsilon \right)^{\frac{1}{2}} $$ 
> 由于$\varepsilon > 0$ 是任意的, 我们有
> $$ T\in  \overline{\left\{ F_{ij}: i,j\in \mathbb{N}  \right\}}\subseteq \overline{F\left(\ell ^{2}\right)}= C\left(\ell ^{2}\right) $$ 
> 
