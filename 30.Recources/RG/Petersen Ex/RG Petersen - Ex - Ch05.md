...
建议将题目分为四个阶段来做。标有 ⭐ 的是**极其重要、博资考/丘赛高频考点**，必须独立完整写出证明并牢记结论。

#### 第一阶段：局部几何与解析工具（法坐标系、Hessian与曲率）
这一阶段训练你的硬计算能力和对局部几何的直觉。在丘赛中，经常需要写出度量在法坐标系下的 Taylor 展开来估计体积或能量。

*   **⭐ 5.9.42 (Riemann & Gauss 度量展开)**: **必做！** 这是整本书最重要的局部计算题之一。用曲率张量写出度量的 Taylor 展开 $g_{kl} = \delta_{kl} + \frac{1}{3}R_{ikjl}x^ix^j + O(|x|^3)$ 是黎曼几何的基本功。
*   **⭐ 5.9.43 (A. Gray 体积展开公式)**: **必做！** 结合上一题，计算测地小球的体积展开式，深刻理解纯量曲率（Scalar Curvature）的几何意义。
*   **5.9.27 (距离函数的 Hessian 展开)**: 重要计算。了解 $\text{Hess}(r)$ 和 $\text{Hess}(\frac{1}{2}r^2)$ 在法坐标系下的渐近行为。
*   **5.9.30 (识别法坐标系)**: 帮助你从定义上理解法坐标系的本质。
*   **⭐ 5.9.21 (函数的凸性与 Hessian)**: 建立函数沿着测地线的二阶导数与流形上 Hessian 的关系。非常基础且重要。

#### 第二阶段：全局几何结构（李群、齐性空间与 Submersion）
丘赛特别喜欢考具有丰富对称性的空间。李群和 Riemannian Submersion 是构造反例和出题的重灾区。

*   **⭐ 5.9.35 (李群上的双不变度量)**: **重中之重！** 证明测地线就是单参数子群、指数映射的重合等。几乎所有几何Qual都会有一道关于 Lie Group 的题。
*   **5.9.9 & 5.9.14 (齐性流形与迷向)**: 理解等距群可递作用对测地完备性的影响（Hopf-Rinow的推论）。
*   **⭐ 5.9.37 (Hermann 定理)**: Riemannian Submersion 提升完备性的经典定理。
*   **5.9.12 (Submersion 分裂定理)**: Hessian 为零的函数与流形等距分裂（De Rham 分裂定理的初级版本）的关系。
*   **5.9.40 (射影空间的等距群)**: 熟悉 $\mathbb{CP}^n$, $\mathbb{RP}^n$, $H^n$ 的等距群构造，丘赛经常拿这些空间当背景。

#### 第三阶段：完备性与距离函数（Hopf-Rinow定理的延伸）
这一阶段主要考察对测地完备性、度量完备性的拓扑理解。

*   **5.9.5 (完备开子集)**: 简单的拓扑推导，但很考验逻辑严密性。
*   **5.9.6 (Misner 完备性)**: 了解不同的完备性条件。
*   **5.9.10 & 5.9.11 (Warped Product 的完备性)**: **强烈建议做！** 极化乘积（Warped Product）是构造黎曼几何反例的万能工具，自己动手判断完备性很有帮助。
*   **5.9.18 (共形变换与完备性)**: 经典技巧：任何流形都可以通过乘以一个增长极快的共形因子变成完备流形。
*   **⭐ 5.9.23 (到子流形的距离与 Segment)**: 理解管状邻域（Tubular neighborhood）和垂直测地线的核心基础。

#### 第四阶段：变分法与割迹（Cut Locus & Variations）
*   **⭐ 5.9.34 (能量泛函的局部极小值)**: 结合共轭点（这里没直接提，但思路一致）和指数映射的正则性，理解为什么测地线局部极短。
*   **5.9.20 & 5.9.31 (全测地子流形)**: 熟悉 Totally Geodesic 的等价条件和交集性质。
*   **5.9.24 & 5.9.25 (割迹的计算)**: 必须亲手算过平环（Torus）、球面和射影空间的 Cut Locus 长什么样，考试遇到复杂空间才有直觉。
*   **5.9.17 (常曲率空间的射影模型)**: 了解高维双曲空间和球面的 Klein 模型。

---

### 🛑 可以略读、直接记结论或跳过的题目（省时间指南）

为了备考丘赛或博资考，你的时间非常宝贵。以下题目要么过于平凡，要么过于偏向实分析/点集拓扑，建议**跳过或只看结论**：

*   **过于基础/平凡的验算**:
    *   **5.9.2, 5.9.3, 5.9.4**: 链式法则和最基础的坐标系验算，一眼看穿即可，不用动笔。
    *   **5.9.15, 5.9.16**: 距离函数梯度的性质，脑内过一遍即可。
*   **实分析/度量几何细节过重（考试概率低）**:
    *   **5.9.28 (距离函数在单侧的方向导数)**: 这题用到了极繁琐的 Dini 导数和极限估计。在几何考试中，通常只需知道“距离函数Lipschitz且几乎处处可导”即可，这题的证明过程不用死磕。
    *   **5.9.29 (绝对连续曲线的长度)**: 纯度量空间理论，偏实分析，跳过。
*   **点集拓扑细节过重**:
    *   **5.9.41 (微分同胚群的紧致开拓扑)**: 除非你同时在复习拓扑群论，否则这题篇幅太长，且对核心几何直觉提升有限，建议直接记住“等距群 $\text{Iso}(M)$ 是局部紧的李群，且 $\text{Iso}_p$ 是紧群”这个重要结论即可。
*   **其他**:
    *   **5.9.1, 5.9.7, 5.9.8, 5.9.19, 5.9.26, 5.9.33**: 都是关于距离概念的小修小补，当做正文定理的注记看看就行。
    *   **5.9.13**: 补全书中定理 5.6.16 的漏洞（没有原书上下文没法做）。

---

### 💡 针对丘赛/博资考的复习建议

1. **熟练背诵 Taylor 展开**：在丘赛中，遇到与体积、标量曲率、Ricci 曲率相关的不等式，**第一反应**就应该是取 Normal Coordinates，然后使用 5.9.42 和 5.9.43 的展开式进行局部积分/估计。
2. **第一/第二变分公式**：这组题里对第二变分的考察不多，但考试**必考**。建议结合 Petersen 后续章节（或 do Carmo 的书）狂刷第二变分和 Jacobi 场的题目（例如 Synge 定理、Bonnet-Myers 定理的证明逻辑）。
3. **李群的几何**：务必弄清 $\nabla_X Y = \frac{1}{2}[X,Y]$ （对于左不变向量场）这个公式的来龙去脉，以及它如何推导出曲率张量 $R(X,Y)Z = -\frac{1}{4}[[X,Y],Z]$。结合 5.9.35 会让你对这一块如鱼得水。

祝你刷题顺利！如果在证明 5.9.42 (Riemann度量展开) 或者 5.9.35 (李群) 时卡壳了，随时可以来问我，这些题的代数推导是有固定套路的。
