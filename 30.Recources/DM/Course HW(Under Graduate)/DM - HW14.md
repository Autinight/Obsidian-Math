...
> [!proof] 证明
> 必要性：若任意 $X\in T_pM$ 的平行延拓与路径无关，则沿任意无穷小闭曲线的平行移动为恒等。
> 对由 $Y,Z$ 张成的无穷小平行四边形，回路平行移动的一阶非平凡项为
> $$
> R(Y,Z)X.
> $$
> 故
...
> 对任意 $X,Y,Z$ 成立，所以 $R=0$。
>
> 充分性：设 $R=0$，且 $M$ 单连通。任取 $q\in M$ 与道路 $\gamma:p\to q$，定义
> $$
> \widetilde X(q)=P_\gamma X.
> $$
> 因 $M$ 单连通，任意两条同端点道路同伦；因 $R=0$，平行移动在同伦下不变。
> 故 $P_\gamma X$ 只依赖端点 $q$，不依赖道路 $\gamma$。
> 于是 $\widetilde X$ 良定义，且沿任意道路平行。
> 所以 $X\in T_pM$ 生成一个与路径无关的平行切向量场。
