...
## Embedded style references

Do **not** read any existing vault files by default when using this skill. The following filenames are historical style sources only; they indicate the intended family of card styles, but the formatting rules in this skill are the authoritative imported examples/specification:
...
Use only the embedded templates and examples below for format normalization unless the user explicitly asks you to read a specific file, references an `@filename.md`, provides a current note/selection that must be used, or asks for consistency with an existing note.
...
Use this for prerequisite wiki-links, source notes, or related concepts. Do not search/read files just to discover links; only use wiki-links supplied by the user or links that are completely standard and unlikely to be broken.
...
4. Do not read any vault file merely to learn style, terminology, or formatting. Use the embedded templates in this skill as the style authority. Only read files when the user explicitly asks for a specific file, references an `@filename.md`, provides a current note/selection that must be used, or the task cannot be completed correctly without the referenced source.
...
6. Avoid inventing precise wiki-links that are likely broken; plain text is better than a typo-link. Since this skill should not search/read by default, only use wiki-links that the user supplied or that are completely standard in the vault context.
...
8. Use only dollar-sign math delimiters:
   ...
   - display: `$$...$$`
   - never use `\(...\)` or `\[...\]`.
9. Add the YAML tag `generated-by-AI` to every card generated by this skill unless the user explicitly requests no AI-generated tag.
10. Write the note to `00.Inbox/<filename>.md` by default.
11. After writing, verify the created note.
...
- YAML frontmatter includes the tag `generated-by-AI` unless the user explicitly opted out.
...
Verified: frontmatter, generated-by-AI tag, callout syntax, dollar-sign math, and obvious wikilinks.
```
