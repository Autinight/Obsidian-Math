...
- **等距同构**
  ...
  - 完备流形到连通流形的局部等距同构是Riemannian Covering
    ...
    - 所以完备性是为了一致存在的法坐标球, 等距同构是为了法坐标球长得一样, 连通性是为了满射
  - 流形在一族**等距同构**下的**不动点集**的连通分支, 是**完全测地的子流形**
    - 对于等距同构$F\circ \exp = \exp \circ DF$, 即测地线终点的变化取决于射出来测地线的方向变化.
    - 通过这个直觉, 如果$q= \exp_{p}\left(v\right)$是不动点, 由$\exp$在某个局部上同胚, 一开始的方向也不应该变化, 也就是说$v$是$DF_{p}$的不动点.
    - 所以就可以通过$DF_{p}$在$T_{p}M$中的不动点这个线性的无穷小模型, 来从局部打印$\operatorname{Fix}\left(S\right)$, 并且由于直接是用测地线打印的, $\operatorname{Fix}\left(S\right)$甚至是完全测地的 
    - 为了确保维数的稳定, 这里假设了连通性.
