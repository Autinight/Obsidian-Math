...
- **李群上的左不变度量**: 
   ...
   2. 矩阵李群上, 指数映射是左不变向量场的积分曲线. 所以对于一般的李群, 定义左不变向量场的指数映射就是积分曲线.
   3. 左不变向量场$X$的流是
        $$ F^{t}\left(x\right) = R_{\exp \left(tX\right)}\left(x\right)$$ 
        是右平移
...
- **黎曼淹没的几何量计算**: 设$\overline{X}$是$X$的水平提升, $V$ is vertical
   1.  $\left[ V,\overline{X} \right]$ is vertical 
