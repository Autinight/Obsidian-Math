...
- Lie Derivative: 流形上的函数复合向量场的积分曲线, 变成直线上的函数, 泰勒展开的一阶量作为李导数. 或者是复合积分曲线后关于$t$求导. 推广到向量场上, 由于无法直接比较, 让向量场沿着积分曲线跑时间$t$后通过微分拉回到原点. 也就是说, 我们通过$X$生成了一种$Y$的运动方式, 又给出了向量场之间的比较方式, 测量了$Y$自己的样子和让$X$拖着走的样子差多少.  这种方式还可以推广到张量, 因为拉回的语言可以推广到张量. 
  - Lie 导数对张量积有Lebniz律
       $$\mathcal{L}_{X}\left(T\otimes S\right)= \left(\mathcal{L}_{X}T\right)\otimes S+ T\otimes \left(\mathcal{L}_{X}S\right)  $$ 
       基于$\varphi$的群性质, $\varphi _{t}^{*}$与$\mathcal{L}_{X}$可交换.
  - 李导数的Talor展开定义通常比直接定义更好用.
     $$\phi_t^* T = T + t \mathcal{L}_X T + \frac{t^2}{2!} \mathcal{L}_X^2 T + \dots + \frac{t^k}{k!} \mathcal{L}_X^k T + O(t^{k+1})$$
